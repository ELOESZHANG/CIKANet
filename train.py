...
# filepath setting
traindata_dir = f'/home/my/dataset/{satellite.upper()}/train/'
validdata_dir = f'/home/my/dataset/{satellite.upper()}/valid/'
...
def main():
    ...
    #     if isinstance(m, KCA):
    #         KCA_params += m.parameters()
    #     elif isinstance(m, KSA):
    #         KSA_params += m.parameters()
    # target_params = KCA_params + KSA_params
    #
    # target_Params_id = list(map(id, target_params))
    # base_Params = filter(lambda p: id(p) not in target_Params_id, model.parameters())
    #
    # training_parameters = [{'params': target_params, 'lr': lr * 100},
    #                        {'params': base_Params}]
    # optimizer2 = torch.optim.SGD(training_parameters, lr=lr, momentum=0.9)

    # training and validating dataset setting
